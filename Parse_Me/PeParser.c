...
BOOL ReadPeFile(LPCSTR lpFileName, PBYTE* pPE, SIZE_T* sPE) {
	...
	// getting the size of the file using its handle
...
VOID ParsePeFile(PBYTE pPE) {
	...
	// getting the NT-HeaderS from the DOS-Header
	...
	// printing the data directory inside the optional header
	printf("\n\t#####################[ DIRECTORIES ]#####################\n\n");
	...
	// tarversing all the section fond in the PE file
